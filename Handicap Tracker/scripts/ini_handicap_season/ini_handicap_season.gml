...
if !allYear
	{
	var start_date = handicap_date_parse(startDate);
	var start_month = real(start_date[0]);
	var start_day = real(start_date[1]);	
	
	startDate = date_create_datetime(current_year,start_month,start_day,1,1,1);
	
	var end_date = handicap_date_parse(endDate);
	var end_month = real(end_date[0]);
	var end_day = real(end_date[1]);
...
function handicap_date_parse(date) {

var arr = ["",""];
var pos = 0;
for(var i=1;i<string_length(date)+1;i++)
	{
	var char = string_char_at(date,i);
