function draw_stats() {
	...
	var course_name = course_id[| course_data.name];
	...
	if !ds_list_empty(stat_tee[i]) // if there is data in the list
		...
		var col = draw_tee_marker(tee_xx,tee_yy+(tee_ysep*0.5)+(tee_ind*tee_ysep),5,i);
		var tee_str = teebox_list[| i]
		draw_text_height(tee_xx+20,tee_yy+(tee_ind*tee_ysep),tee_str,tee_ysep); // draw tee marker name
	
		// select tee
		if can_select
			{
			if click_region_released(0,tee_yy+(tee_ind*tee_ysep),ww,tee_ysep,true,navbar.main)
				{
				stat_tee_index = i;
				can_select = false;
				}
	...
	if !ds_list_empty(stat_tee[i]) // if there is data in the list
		...
		for(var n=disp_start;n<disp_end;n++)
			...
			var col = draw_tee_marker(xpos,ypos,5,i);
