...
function draw_index_trend() {
	...
	for(var i=pos_start;i<pos_end;i++)
		...
		var pointer = master_score_list[| n];
		var ind = pointer[| score_data.index]; // get the index
		var date = pointer[| score_data.date]; // get the date
		...
		if size != max_disp
			{
			var dir = -1;
			var col = draw_tee_marker(xpos,ypos,5,tee_marker.black); // draw dot
			...
			if click_region(xx+xoff+(i*xsep),yy-hh,ww_off,hh,mb_left,noone)
				...
				var col = draw_tee_marker(xpos,ypos,5,tee_marker.black);
